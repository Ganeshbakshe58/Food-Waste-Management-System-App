      ...
      <ScrollView style={styles.requestCont}>
        {
          ...
          return (<TouchableOpacity onPress={() => { navigation.navigate("CheckInfo",{params:data,othShow:showOther,deliveryStatus:ReqType == "Delivered" ? true :false})}} style={[styles.reqBox, styles.shadow,data.Status == "Delivered" && {borderColor:"#00e6e6",backgroundColor: "#008080"}, styles.shadow,data.Status == "Accept" && {borderColor:"lightgreen",backgroundColor: "#39ac73"}]} key={data._id} >
              ...
              <View style={styles.box1}>
                ...
                <Text style={styles.reqDetail}><Text style={[data.Status == "Pending"&&{color:'#0088cc'},data.Status == "Accept"&&{color:'lightgreen'},data.Status == "Delivered"&&{color:'#00e6e6'},{ fontWeight: "700" }]}><FontAwesomeIcon color='#e6f2ff' size={11} icon = {faUserAlt}></FontAwesomeIcon> Name : </Text>{data.Name}</Text>
                <Text style={styles.reqDetail}><Text style={[data.Status == "Pending"&&{color:'#0088cc'},data.Status == "Accept"&&{color:'lightgreen'},data.Status == "Delivered"&&{color:'#00e6e6'},{ fontWeight: "700" }]}><FontAwesomeIcon color='#e6f2ff' size={11} icon = {faListNumeric}></FontAwesomeIcon> FeedCount : </Text> {data.Feedcount}</Text>
                <Text style={styles.reqDetail}><Text style={[data.Status == "Pending"&&{color:'#0088cc'},data.Status == "Accept"&&{color:'lightgreen'},data.Status == "Delivered"&&{color:'#00e6e6'},{ fontWeight: "700" }]}><FontAwesomeIcon color='#e6f2ff' size={11} icon = {faCity}></FontAwesomeIcon> City : </Text> {data.City}</Text>
              </View>
              <View style={styles.box2}>
                {/* <TouchableOpacity><Text style={[styles.reqStateBtn, { color: data.Status == "Pending" ? "red" : "green" }]}>{data.Status}</Text></TouchableOpacity> */}
                <Text style={[styles.date,data.Status == "Accept" && {color:"lightgreen"},styles.date,data.Status == "Delivered" && {color:"#00e6e6"}]}><FontAwesomeIcon color={data.Status == "Pending"?'#0088cc':"white"} size={10} icon = {faCalendarAlt}></FontAwesomeIcon> {data.Date}  <FontAwesomeIcon color={data.Status == "Pending"?'#0088cc':"white"} size={10} icon = {faStopwatch}></FontAwesomeIcon>{data.Time}</Text>
              </View>
            </TouchableOpacity>
          )
        })
